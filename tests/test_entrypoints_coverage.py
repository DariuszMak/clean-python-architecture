...
def test_add_batch_returns_201(flask_client) -> None:
    client, _ = flask_client
    FAKE_BUS.reset_mock()
    resp = client.post(
        "/add_batch",
        json={"ref": "b1", "sku": "SMALL-TABLE", "qty": 10, "eta": None},
    )
...
def test_add_batch_with_eta(flask_client) -> None:
    client, _ = flask_client
    FAKE_BUS.reset_mock()
    eta_str = (datetime.now(tz=UTC).date() + timedelta(days=5)).isoformat()
    resp = client.post(
        "/add_batch",
        json={"ref": "b2", "sku": "LAMP", "qty": 5, "eta": eta_str},
    )
    assert resp.status_code == 201


def test_allocate_returns_202(flask_client) -> None:
    client, _ = flask_client
    FAKE_BUS.reset_mock()
    resp = client.post(
        "/allocate",
        json={"orderid": "o1", "sku": "SMALL-TABLE", "qty": 3},
    )
...
def test_allocate_returns_400_on_invalid_sku(flask_client) -> None:
    ...
    resp = client.post(
        "/allocate",
        json={"orderid": "o1", "sku": "GHOST", "qty": 3},
    )
...
def test_allocations_view_returns_200(flask_client) -> None:
    ...
    with mock.patch.object(
        flask_module,
        "allocations",
        return_value=[{"sku": "SMALL-TABLE", "batchref": "b1"}],
    ):
        resp = client.get("/allocations/o1")
    assert resp.status_code == 200
    assert resp.get_json() == [{"sku": "SMALL-TABLE", "batchref": "b1"}]


def test_allocations_view_returns_404_when_empty(flask_client) -> None:
    client, flask_module = flask_client
    FAKE_BUS.reset_mock()
    with mock.patch.object(flask_module, "allocations", return_value=[]):
...
def test_handle_change_batch_quantity(consumer_module) -> None:
    consumer, _ = consumer_module
    FAKE_BUS.reset_mock()
    msg = _make_redis_message({"batchref": "b1", "qty": 25})
...
def test_main_subscribes_and_handles_messages(consumer_module) -> None:
